...
old = """    selectedAdminContentFile: null
  };"""
new = """    selectedAdminContentFile: null,
    adminCourseId: '',
    adminCourseStatus: '',
    adminCourseStore: '',
    adminCourseSearch: '',
...
start = s.index('  function renderAdminCourses() {')
end = s.index('\n  function bindAdminAccordions(root) {', start)
replacement = r'''  function adminCourseRows(packageId) {
...
old = """    $('adminPeoplePanel').hidden = tab !== 'people';
    $('adminCoursesPanel').hidden = tab !== 'courses';"""
new = """    $('adminPeoplePanel').hidden = tab !== 'people';
    $('adminCoursesPanel').hidden = tab !== 'courses';
    if ($('adminSearchForm')) $('adminSearchForm').hidden = tab !== 'people';"""
assert s.count(old) == 1, s.count(old)
s = s.replace(old, new, 1)

old = """    $('adminSearch').oninput = () => { renderAdminPeople(); renderAdminCourses(); };"""
new = """    $('adminSearch').oninput = renderAdminPeople;"""
assert s.count(old) == 1, s.count(old)
s = s.replace(old, new, 1)
...
/* V1.0 admin course filter view */
.admin-course-filter-card{display:grid;gap:14px}
.admin-course-filter-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:10px;padding:14px;border:1px solid var(--line);border-radius:16px;background:#fff}
.admin-course-filter-grid .field-group{margin:0}
.admin-course-summary-row{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}
.admin-course-stat{appearance:none;border:1px solid var(--line);border-radius:14px;background:#fff;padding:11px 12px;text-align:left;cursor:pointer;color:inherit}
.admin-course-stat span{display:block;color:var(--muted);font-size:.78rem;margin-bottom:3px}
.admin-course-stat strong{font-size:1.25rem}
.admin-course-stat.is-active{border-color:#c97943;background:var(--soft)}
.admin-course-search-row{display:flex;align-items:center;gap:10px}
.admin-course-search-row input{flex:1;min-width:0}
.admin-course-result-list{display:grid;gap:8px}
.admin-course-person{border:1px solid var(--line);border-radius:14px;background:#fff;overflow:hidden}
.admin-course-person .person-package__toggle{width:100%}
.admin-course-person .person-package__body{border-top:1px solid var(--line)}
.admin-course-empty{margin-top:0}
@media (max-width:900px){.admin-course-filter-grid{grid-template-columns:1fr 1fr}.admin-course-summary-row{grid-template-columns:1fr 1fr}}
@media (max-width:600px){.admin-course-filter-grid{grid-template-columns:1fr}.admin-course-search-row{display:grid}.admin-course-summary-row{grid-template-columns:1fr 1fr}}
'''
css.write_text(c, encoding='utf-8')

idx = Path('index.html')
h = idx.read_text(encoding='utf-8')
h = h.replace('styles.css?v=1.0', 'styles.css?v=1.0-adminfilter-20260828', 1)
h = h.replace('app.js?v=1.0', 'app.js?v=1.0-adminfilter-20260828', 1)
