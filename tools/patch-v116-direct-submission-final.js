...
one("${direct ? '直接放在課程內' : `${lesson.required ? '必修' : '選修'}｜${escapeHtml(ruleText(lesson))}｜${adminEnabledText(lesson.enabled)}｜適用：${escapeHtml(applicabilityLabel(lesson))}${lesson.submissionMode && lesson.submissionMode !== '不需要' ? `｜作業：${escapeHtml(lesson.submissionMode)}` : ''}`}",
    "${direct ? escapeHtml(directLessonMeta(lesson)) : `${lesson.required ? '必修' : '選修'}｜${escapeHtml(ruleText(lesson))}｜${adminEnabledText(lesson.enabled)}｜適用：${escapeHtml(applicabilityLabel(lesson))}${lesson.submissionMode && lesson.submissionMode !== '不需要' ? `｜作業：${escapeHtml(lesson.submissionMode)}` : ''}`}",
...
one(
"${escapeHtml(x.store || '')}｜${escapeHtml(x.courseTitle)} → ${escapeHtml(x.lessonTitle)}",
"${escapeHtml(x.store || '')}｜${escapeHtml(x.courseTitle)} → ${escapeHtml(x.lessonTitle === '__PACKAGE_DIRECT__' ? '課程回傳' : x.lessonTitle)}",
...
console.log('patched app.js', Buffer.byteLength(s), 'bytes');
