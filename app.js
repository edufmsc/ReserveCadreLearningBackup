(() => {
  ...
  function renderPackageCard(pkg) {
    ...
    const meta = [
      ...
      pkg.completionRule === '任一必修子課程完成' ? '任一必修完成即可' : '',
      dueText ? `截止 ${dueText}` : '',
      overdue ? '已逾期' : ''
    ].filter(Boolean).join('｜');
    ...
    return `<article class="package-card">
      <div class="package-header"><div class="package-topline"><div><h3 class="package-title">${escapeHtml(pkg.title)}</h3>${meta ? `<p class="package-meta">${escapeHtml(meta)}</p>` : ''}</div><div class="package-status-stack">${overdue ? '<span class="tag tag--danger">已逾期</span>' : ''}${statusTag(summary.status)}</div></div>
  ...
  function buildCriteria(lesson) {
    ...
    if (!required) return `<div class="criteria-panel"><strong>完成條件</strong><div class="criteria-list"><span class="criteria-item is-pass">✓ 無額外閱讀門檻</span></div></div>`;
  ...
  function renderLessonPage(preview = false) {
    ...
    $('lessonPackageName').textContent = pkg.title;
    $('lessonTitle').textContent = visibleLessonTitle(lesson);
    $('lessonMeta').innerHTML = (preview ? '<div class="preview-banner">學員視角預覽</div>' : '') + `<span class="tag">${escapeHtml(contentTypes(lesson))}</span><span class="tag ${lesson.required ? 'tag--warning' : 'tag--muted'}">${lesson.required ? '必修' : '選修'}</span>`;
  ...
  function renderManagePackage(pkg, assignments) {
    ...
    const manageMeta = [clean(pkg.description), adminEnabledText(pkg.enabled), `已指派 ${activeAssign} 人`].filter(Boolean).join('｜');
    return `<article class="manage-card"><div class="manage-card__head"><div class="manage-card__title"><strong>${escapeHtml(pkg.title)}</strong><small>${escapeHtml(manageMeta)}</small><div style="margin-top:7px">${publishBadge(pkg)}</div>${errors.length ? `<div class="manage-warning">${escapeHtml(errors.slice(0,3).join('；'))}${errors.length > 3 ? '…' : ''}</div>` : ''}</div>
      ...
      <div class="manage-card__body" data-manage-body="${escapeHtml(pkg.id)}" ${open ? '' : 'hidden'}>${directRow}${lessonRows || '<div class="manage-empty">尚未建立子課程</div>'}</div></article>`;
  }
  ...
  function openContentEditor(content, lessonId = '', directPackageId = '') {
    ...
    $('testContentLinkButton').onclick = () => {
      ...
      if (type === 'PDF') { showToast('請儲存後預覽 PDF。'); return; }
      if (type === 'FILE') {
  ...
  function ensureSubmissionTab() {
    ...
    if (!$('adminSubmissionPanel')) {
      ...
      panel.innerHTML = '<div class="panel-heading"><div><h2>作業回傳</h2></div></div><div id="submissionAdminBody"></div>';
  ...
  async function reviewSubmission(id, decision, button, fromStatus = '') {
